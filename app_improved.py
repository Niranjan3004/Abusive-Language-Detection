...
def predict():
    try:
        ...
        # Check if obviously safe
        if is_safe_by_heuristics(text):
            result = {
                'text': text,
                'label': 'non-abusive',
                'confidence': 0.95,
        ...
        # Additional safety check: if confidence is low and word seems safe, default to non-abusive
        if confidence.item() < 0.7 and len(text.split()) <= 3:
            predicted = torch.tensor([0])  # Force non-abusive for short, uncertain texts
            abuse_probs = torch.tensor([[0.8, 0.2]])
...
if __name__ == '__main__':
    ...
    print(f"✓ Added safety heuristics for common words")
